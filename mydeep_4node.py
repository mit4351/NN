...
def init_wait(weight_init_std = 0.01):
    ...
    result['W3'] = weight_init_std * np.random.randn(hiddn2, hiddn3) * 0.01
    result['b3'] = np.random.randint(-100,100,hiddn3) * 0.0001
    #result['b3'] = np.zeros(hiddn3)

    result['W4'] = weight_init_std * np.random.randn(hiddn3, 10) * 0.01
    result['b4'] = np.random.randint(-100,100,10) * 0.0001
    #result['b4'] = np.zeros(10)

    return result

def save_network(file_name="mit_ht3.pkl"):
    global W1,W2,W3,W4,network
    global b1,b2,b3,b4
    network['W1'], network['W2'], network['W3'], network['W4'] = W1, W2, W3, W4
    network['b1'], network['b2'], network['b3'], network['b4'] = b1, b2, b3, b4
...
# 誤差逆伝播
def back_propagation(x, z1, z2, z3, y, t):
    global W1,W2,W3,W4
    ...
    #softmaxの交差エントロピー
    delta4 = y - npdata
    grad_W4 = z3.T.dot(delta4)

    #3層目のネットワークのパラメータの勾配
    sigmoid_dash3 = z3 * (1 - z3)
    delta3 = delta4.dot(W4.T) * sigmoid_dash3
...
def runing(tr_cnt, x, t, batch_size = 100):
    ...
    for i in range(0, len(x), batch_size):
        ...
        y_batch, z1_batch, z2_batch, z3_batch  = predict(x_batch)
        p = np.argmax(y_batch, axis=1) #1次元目の要素ごとに最大値を抽出
        accuracy_cnt += np.sum(p == t[i:i+batch_size])
        back_propagation(x_batch, z1_batch, z2_batch, z3_batch, y_batch, t_batch)
