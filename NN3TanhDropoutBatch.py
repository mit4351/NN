...
def forward(x, W1, W2, W3, b1, b2, b3, dropout, training=False):
    a1 = np.dot(x, W1) + b1
    z1 = np.tanh(a1)
    a2 = np.dot(z1, W2) + b2
    z2 = np.tanh(a2)
...
def get_sample(X, Y, batch_size=100):
    '''
    for x, y in zip(X, Y):
...
def testing(network, X_test, Y_test, dropout = 1.0):
    ...
    for x, t in get_sample(X_test, Y_test, 1):
        _, _, y, _ = forward(x, W1, W2, W3, b1, b2, b3, dropout, training=False)
...
def training(network, pX_train, pY_train, X_validation, Y_validation, n_samples=5000):
    n_epochs = 20
    if n_samples ==0:
    ...
    for epoch in range(n_epochs):
        ...
        # Training
        accuracy_cnt = 0
        for x, t in get_sample(X_train, Y_train, batch_size):
            z1, z2, y, m2 = forward(x, W1, W2, W3, b1, b2, b3, dropout, training=True)
            dC_dW1, dC_dW2, dC_dW3 = backward(x, z1, z2, y, m2, t, W1, W2, W3)
            W1, W2, W3 = update(W1, W2, W3, dC_dW1, dC_dW2, dC_dW3, learning_rate)
            yy = np.argmax(y, axis=1) #1次元目の要素ごとに最大値を抽出
            tt = np.argmax(t, axis=1) #1次元目の要素ごとに最大値を抽出
            accuracy_cnt += np.sum(yy == tt)
            #accuracy_cnt += np.sum(np.argmax(y) == np.argmax(t))
            #print("y.shape=%s ::t.shape=%s  ::y=%s ::t=%s  ::yy=%s ::tt=%s" % (y.shape, t.shape, np.argmax(y), np.argmax(t), yy, tt))
        ...
        cost = 0.
        accuracy_cnt2 = 0
        for x, t in get_sample(X_validation, Y_validation, batch_size):
            _, _, y, _ = forward(x, W1, W2, W3, b1, b2, b3, dropout, training=False)
            cost += mean_squared_error(y, t)
            #accuracy_cnt2 += np.sum(np.argmax(y) == np.argmax(t))
            yy = np.argmax(y, axis=1) #1次元目の要素ごとに最大値を抽出
            tt = np.argmax(t, axis=1) #1次元目の要素ごとに最大値を抽出
            accuracy_cnt2 += np.sum(yy == tt)
...
def plotDisplay(score_history, score_history2, tDataCnt, tDataCnt2):
    plt.title("3層NN (Training:%5d  Validation:%5d)"  % (tDataCnt, tDataCnt2))
